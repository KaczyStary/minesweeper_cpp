...
void MinesweeperBoard::initGameMode() {
    gameMode=EASY;
}
...
void MinesweeperBoard::generateMines(double numberOfMinesFor){
    ...
    int numberOfMines=0;
    numberOfMines= boardHeight * boardWidth * numberOfMinesFor;
...
void MinesweeperBoard::setMines() {
    ...
    if (gameMode==EASY){
        numberOfMinesFor=0.1;
    }

    if (gameMode==NORMAL){
        numberOfMinesFor=0.2;
    }

    if (gameMode==HARD){
        numberOfMinesFor=0.3;
    }

    generateMines(numberOfMinesFor);
...
int MinesweeperBoard::minesOnBoardCounterForDev() {
    int minesHolder=0;

    for (int forHeight = 0; forHeight < boardHeight; ++forHeight) {
        for (int forWidth = 0; forWidth < boardWidth; ++forWidth) {

            if (boardVector[forHeight][forWidth].hasMine){
                minesHolder++;
            }

            if (boardVector[forHeight][forWidth].hasMine&&boardVector[forHeight][forWidth].hasFlag){
                minesHolder--;
            }
